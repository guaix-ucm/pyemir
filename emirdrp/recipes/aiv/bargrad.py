...
def exvp(pos_x, pos_y):
    ...
    cf  = (0.1944 * 1 / 3600.0) * numpy. pi / 180.0
    # ra = numpy.sqrt(($1-1024.5)**2+($2-1024.5)**2)
    ...
    nx1 = rr1 * ra * numpy.sin(thet) + center[0]
    ny1 = rr1 * ra * numpy.cos(thet) + center[1]
    return nx1, ny1
...
class BarDetectionDistRecipe(EmirRecipe):
    ...
    def run(self, rinput):
        ...
        self.logger.debug('computing kernel weights')
        for ks in [3, 5, 7, 9]: # Different kernel sizes
            # S and G kernel for derivative
            kw = ks * (ks*ks-1) / 12.0
            coeffs_are = -numpy.arange((1-ks)//2, (ks-1)//2 + 1) / kw
            self.logger.debug('kernel %d, weights are %s', ks, coeffs_are)
            sg_kernel[ks] = coeffs_are
        ...
        for ks in [3, 5, 7, 9]: # Different kernel sizes
            ...
            arr_deriv = convolve1d(arr_median, sg_kernel[ks], axis=-1)
            #
            self.logger.debug('derive image in Y direction (with kernel=3)')
            arr_deriv_alt = convolve1d(arr_median_alt, sg_kernel[3], axis=0)
            ...
            for coords in barstab:
                ...
                centery, xpos, fwhm, st = char_bar_peak_l(
                    arr_deriv,
                    prow,
                    bstart,
                    bend,
