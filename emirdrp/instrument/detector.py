...
class EMIR_Detector_1(nIRDetector):
    def __init__(self, dark=0.25, flat=1.0, bad_pixel_mask=None):
        ...
        wdepth = 55292
        saturation = 57000
        channels = [Channel((slice(0, 2048), slice(0, 2048)),
                            gain, ron, pedestal,
                            wdepth, saturation)
                    ]
        super(EMIR_Detector_1, self).__init__(
            (2048, 2048), channels,
            dark=dark, flat=flat,
...
class EMIR_Detector_4(nIRDetector):
    def __init__(self, dark=0.25, flat=1.0, bad_pixel_mask=None):
        ...
        channels = [Channel(*vs) for vs in zip(QUADRANTS,
                                               gain, ron, pedestal,
                                               wdepth, saturation
                                               )
                    ]

        super(EMIR_Detector_4, self).__init__(
            (2048, 2048), channels,
            dark=dark, flat=flat,
            bad_pixel_mask=bad_pixel_mask
            )
