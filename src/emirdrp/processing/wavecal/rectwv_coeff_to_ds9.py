...
def rectwv_coeff_to_ds9(rectwv_coeff, limits=None, rectified=False, numpix=100):
        ...
        if islitlet not in rectwv_coeff.missing_slitlets:
            ...
            if rectified:
                ...
                ds9_output += (
                    ...
                    'roman"\n'.format(xdum_label, ydum_label, islitlet, colorbox)
                )
            else:
                if limits == "frontiers":
                    pol_lower = Polynomial(dumdict["frontier"]["poly_coef_lower"])
                    pol_upper = Polynomial(dumdict["frontier"]["poly_coef_upper"])
                else:
...
def spectral_lines_to_ds9(rectwv_coeff, spectral_lines=None, rectified=False):
        ...
        if islitlet not in rectwv_coeff.missing_slitlets:
            ...
            if rectified:
                ...
                for xx1_, xx2_, yy1_, yy2_ in zip(xx1, xx2, yy1, yy2):
                    ds9_output += "line {0} {1} {2} {3}".format(xx1_, yy1_, xx2_, yy2_)
                    ds9_output += " # color={0}\n".format(colorbox)
                # slitlet label
                pol_lower = Polynomial(dumdict["spectrail"]["poly_coef_lower"])
                pol_upper = Polynomial(dumdict["spectrail"]["poly_coef_upper"])
...
def main(args=None):
    ...
    save_spectral_lines_ds9(rectwv_coeff=rectwv_coeff, debugplot=args.debugplot)
