...
def rectwv_coeff_from_mos_library(
    ...
    for islitlet in list_valid_islitlets:
        ...
        ioffset = int(ydummid - (tmpdict["bb_ns1_orig"] + tmpdict["bb_ns2_orig"]) / 2.0)
        ...
        xdum1 = corr_yrect_a + corr_yrect_b * tmpdict["y0_frontier_lower"]
        xdum2 = corr_yrect_a + corr_yrect_b * tmpdict["y0_frontier_upper"]
            ...
            dumdict.update(
                {
                    ...
                    "x0_reference": float(EMIR_NAXIS1) / 2.0 + 0.5,
                    "y0_frontier_lower_expected": expected_y0_lower_frontier(islitlet),
                    "y0_frontier_upper_expected": expected_y0_upper_frontier(islitlet),
                }
            )
...
def main(args=None):
    ...
    parser.add_argument(
        "--out_json",
        required=True,
        help="Output JSON file with calibration computed for " "the input FITS file",
