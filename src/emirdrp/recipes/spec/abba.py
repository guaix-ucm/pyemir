...
def compute_wcs_offsets(hduls):
    ...
    for i in range(nimages):
        ...
        ra, dec = wcsh.wcs_pix2world(EMIR_NAXIS1 / 2 + 0.5, EMIR_NAXIS2 / 2 + 0.5, 1)
        ra_, dec_ = wcsh.wcs_pix2world(EMIR_NAXIS1 / 2 + 0.5, EMIR_NAXIS2 / 2 + 1.5, 1)
        if i == 0:
...
class ABBASpectraRectwv(EmirRecipe):
    ...
    def run(self, rinput):
        ...
        try:
            npix_removed_near_ohlines = int(dict_rtas["npix_removed_near_ohlines"])
        except KeyError:
        ...
        for i, char in enumerate(full_set):
            ...
            reduced_mos_image = apply_rectwv_coeff(reduced_image, list_rectwv_coeff[i])
            if save_individual_images != 0:
                self.save_intermediate_img(
                    reduced_mos_image,
                    "reduced_mos_image_" + char + "_" + frame.filename[:10] + ".fits",
                )
        ...
        if self.intermediate_results:
            for imode, outfile in enumerate(
                ...
                median_image = median_slitlets_rectified(reduced_mos_abba, mode=imode)
    ...
    def extract_tags_from_obsres(self, obsres, tag_keys):
        ...
        for frame in obsres.frames:
            ref_img = frame.open()
            tag = self.extract_tags_from_ref(ref_img, tag_keys, base=obsres.labels)
...
class ABBASpectraFastRectwv(EmirRecipe):
    ...
    def run(self, rinput):
        ...
        if nimages % pattern_length != 0:
            raise ValueError(
                "Number of images is not a multiple of expected pattern "
                "length: {}, {}".format(nimages, pattern_length)
            )
        nsequences = nimages // pattern_length
        if nsequences == 0:
