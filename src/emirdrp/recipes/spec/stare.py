...
from emirdrp.processing.wavecal.synthetic_lines_rawdata import synthetic_lines_rawdata
...
class StareSpectraRecipe(EmirRecipe):
    ...
    def run(self, rinput):
        ...
        hdulist = basic_processing_with_combination(rinput, flow, method=sigmaclip)
        ...
        if rinput.master_rectwv:
            ...
            if self.intermediate_results:
                for imode, outfile in enumerate(
                    ...
                    median_image = median_slitlets_rectified(stare_image, mode=imode)
    ...
    def run(self, rinput):
        ...
        if rinput.refine_wavecalib_mode != 0:
            ...
            if rinput.global_integer_offsets_mode == "auto":
                ...
                self.logger.info("global_float_offset_x_pix..: {}".format(-shifts[1]))
        ...
        if self.intermediate_results:
            for imode, outfile in enumerate(
                ...
                median_image = median_slitlets_rectified(reduced_mos, mode=imode)
                self.save_intermediate_img(median_image, outfile + ".fits")

        # save results in results directory
        self.logger.info("end rect.+wavecal. reduction of stare spectra")
        result = self.create_result(reduced_mos=reduced_mos, rectwv_coeff=rectwv_coeff)
        return result
