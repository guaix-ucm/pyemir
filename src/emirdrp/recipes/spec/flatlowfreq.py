...
from emirdrp.processing.wavecal.rectwv_coeff_to_ds9 import save_spectral_lines_ds9
...
class SpecFlatLowFreq(EmirRecipe):
    ...
    nwindow_x_median = Parameter(
        301,
        description="Window size to smooth the flatfield in the spectral " "direction",
        optional=True,
    )
    nwindow_y_median = Parameter(
        11,
        description="Window size to smooth the flatfield in the spatial " "direction",
        optional=True,
    )
    ...
    def run(self, rinput):
        ...
        self.logger.info(
            "nwindow_x_median......................: {}".format(rinput.nwindow_x_median)
        )
        self.logger.info(
            "nwindow_y_median......................: {}".format(rinput.nwindow_y_median)
        )
        self.logger.info(
        ...
        self.logger.info("Number of images with lamp ON.........: {}".format(n_on))
        self.logger.info("Number of images with lamp OFF........: {}".format(n_off))
        self.logger.info("Total number of images................: {}".format(nimages))
        if n_on == 0:
        ...
        for islitlet in list(range(1, EMIR_NBARS + 1)):
            if islitlet in list_valid_islitlets:
                ...
                yy0 = slt.corr_yrect_a + slt.corr_yrect_b * spectrail(slt.x0_reference)
                ...
                sp_median = ndimage.median_filter(sp_collapsed, 5, mode="nearest")
                ...
                if abs(slt.debugplot) % 10 != 0:
                    ...
                    ax.plot([1, naxis1_slitlet2d], 2 * [y_threshold], label="threshold")
                    # ax.plot(xknots, yknots, 'o', label='knots')
                    ax.legend()
                    ax.set_ylim(-0.05 * ymax_spmedian, 1.05 * ymax_spmedian)
                    pause_debugplot(slt.debugplot, pltshow=True, tight_layout=True)
                ...
                slitlet2d_unrect_clipped = slt.rectify(
                    ...
                    resampling=2,
                    inverse=True,
                    subtitle="unrectified, filled with median spectrum " "(clipped)",
                )
                ...
                for j in range(naxis1_slitlet2d):
                    for i in range(naxis2_slitlet2d):
                        ...
                        if den == 0:
                            ...
                            slitlet2d_norm_clipped[i, j] = slitlet2d[i, j] / den
                ...
                for j in range(EMIR_NAXIS1):
                    ...
                    image2d_flatfielded[(n1 - 1) : n2, j] = slitlet2d_norm_smooth[
                        (nn1 - 1) : nn2, j
                    ]
        ...
        filtered = np.where(image2d_flatfielded < rinput.minimum_value_in_output)
        image2d_flatfielded[filtered] = 1.0

        # set pixels above maximum value to 1.0
        filtered = np.where(image2d_flatfielded > rinput.maximum_value_in_output)
        image2d_flatfielded[filtered] = 1.0
