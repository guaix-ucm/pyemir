...
def main(args=None):
    ...
    for ifile in range(nfiles):
        ...
        for islitlet in list_valid_islitlets:
            ...
            ttd_order_modeled = tmpdict[islitlet - 1]["ttd_order_longslit_model"]
    ...
    if len(poldeg_no_duplicates) != 1:
        ...
        raise ValueError("poldeg is not constant in frontiers and " "spectrails!")
            ...
            for icoef in range(poldeg_wavecal + 1):
                ...
                for ifile in range(nfiles):
                    ...
                    if cij is not None:
                        ...
                        if abs(args.debugplot) >= 10:
                            print(
                                "Warning: using wpoly_coeff_longslit_model"
                                + " for "
                                + cslitlet
                                + " in file "
                                + list_json_files[ifile].filename
                            )
                outdict["contents"][cslitlet]["list_wpoly_coeff_" + ccoef] = list_cij
        else:
            islitlet_progress(islitlet, EMIR_NBARS, ignore=True)
    print("OK!")
