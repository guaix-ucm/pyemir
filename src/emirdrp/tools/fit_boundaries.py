...
def integrity_check(bounddict, max_dtu_offset):
    ...
    valid_slitlets = ["slitlet" + str(i).zfill(2) for i in range(1, EMIR_NBARS + 1)]
    ...
    for tmp_slitlet in read_slitlets:
        ...
        for tmp_dateobs in read_dateobs:
            ...
            if tmp_dict["boundary_xmax_lower"] <= tmp_dict["boundary_xmin_lower"]:
                print("ERROR:")
                print("grism...:", grism)
                print("slitlet.:", tmp_slitlet)
                print("date_obs:", tmp_dateobs)
...
def exvp_scalar(x, y, x0, y0, c2, c4, theta0, ff):
    ...
    # center)
    rdist = 1 + c2 * 1.0e4 * r_rad**2 + c4 * 1.0e9 * r_rad**4
...
def expected_distorted_boundaries(
    islitlet,
    csu_bar_slit_center,
    borderlist,
    params,
    ...
    c2, c4, ff, slit_gap, slit_height, theta0, x0, y0, y_baseline = return_params(
        islitlet, csu_bar_slit_center, params, parmodel
    )
...
def overplot_boundaries_from_params(
    ax,
    params,
    parmodel,
    list_islitlet,
...
def overplot_frontiers_from_params(
    ax,
    params,
    parmodel,
    list_islitlet,
...
def save_boundaries_from_params_ds9(
    ...
    ds9_file.write("#\n# global_offset_x_pix: {0}\n".format(global_offset_x_pix))
    ds9_file.write("# global_offset_y_pix: {0}\n#\n".format(global_offset_y_pix))
    ...
    for islitlet, csu_bar_slit_center in zip(list_islitlet, list_csu_bar_slit_center):
        ...
        ds9_file.write("#\n# islitlet...........: {0}\n".format(islitlet))
        ds9_file.write("# csu_bar_slit_center: {0}\n".format(csu_bar_slit_center))
        pol_lower_expected = expected_distorted_boundaries(
            islitlet,
            csu_bar_slit_center,
            [0],
...
def save_frontiers_from_params_ds9(
    params,
    parmodel,
    list_islitlet,
    list_csu_bar_slit_center,
...
def bound_params_from_dict(bound_param_dict):
    ...
    for mainpar in EXPECTED_PARAMETER_LIST:
        ...
        if bound_param_dict["meta_info"]["parmodel"] == "longslit":
            ...
            params.add(mainpar, value=dumdict["value"], vary=dumdict["vary"])
...
def main(args=None):
    ...
    parser.add_argument(
        "bounddict",
        help="Input JSON boundary file with fits to " "continuum-lamp exposures",
    ...
    parser.add_argument(
        "--parmodel",
        required=True,
        help="Parameter model: multislit (default) or " "longslit",
        default="multislit",
    ...
    parser.add_argument(
        "--tolerance",
        help="Tolerance for Nelder-Mead minimization process " "(default=1E-7)",
        type=float,
        default=1e-7,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--background_image",
        help="Optional FITS image to display as background " "image",
    ...
    if args.pickle_input is not None:
        ...
        result = fitter.scalar_minimize(method="Nelder-Mead", tol=args.tolerance)
        pickle.dump(result, open("dum.pickle", "wb"))

    global_residual = fun_residuals(
        result.params,
        args.parmodel,
        bounddict,
        args.shrinking_factor,
    ...
    fitted_bound_param["meta_info"]["creation_date"] = datetime.now().isoformat()
    fitted_bound_param["meta_info"]["description"] = "fitted boundary parameters"
    ...
    if args.debugplot % 10 != 0:
        ...
        pause_debugplot(debugplot=args.debugplot, pltshow=True, tight_layout=True)
